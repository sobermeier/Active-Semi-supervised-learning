...
mean, std = {}, {}
mean['bioscan'] = [0.485, 0.456, 0.406]
std['bioscan'] = [0.229, 0.224, 0.225]

def get_bioscan(args, alg, name, num_labels, num_classes, data_dir='./data', include_lb_to_ulb=True, lb_index=None, ulb_index=None, no_labels = False):
	...
	transform_weak = transforms.Compose([
		...
		transforms.ToTensor(),
		transforms.Normalize(mean['bioscan'], std['bioscan'])
	])
	...
	data_dir = os.path.join(data_dir, "bioscan")
	...
	print("validation")
	val_dset = SaloonDataset(root=os.path.join(data_dir, f"validation"), transform=transform_val, alg=alg, ulb=False)



	return lb_dset, ulb_dset, eval_dset, val_dset
