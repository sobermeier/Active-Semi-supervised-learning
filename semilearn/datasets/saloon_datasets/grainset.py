...
mean, std = {}, {}
mean['grainset'] = [0.485, 0.456, 0.406]
std['grainset'] = [0.229, 0.224, 0.225]

def get_grainset(args, alg, name, num_labels, num_classes, data_dir='./data', include_lb_to_ulb=True, lb_index=None, ulb_index=None, no_labels = False):
	...
	transform_weak = transforms.Compose([
		...
		transforms.ToTensor(),
		transforms.Normalize(mean['grainset'], std['grainset'])
	])
	...
	data_dir = os.path.join(data_dir, "grainset_full")
	if name == "grainset_maize":
		data_dir = os.path.join(data_dir, "maize")
	elif name == "grainset_rice":
		data_dir = os.path.join(data_dir, "rice")
	elif name == "grainset_wheat":
		data_dir = os.path.join(data_dir, "wheat")
	elif name == "grainset_sorghum":
		data_dir = os.path.join(data_dir, "sorg")

	dataset = SaloonDataset(root=os.path.join(data_dir, f"train"), transform=transform_weak, ulb=False, alg=alg, strong_transform=transform_strong)
	...
	print("eval")
	eval_dset = SaloonDataset(root=os.path.join(data_dir, f"test"), transform=transform_val, alg=alg, ulb=False)
