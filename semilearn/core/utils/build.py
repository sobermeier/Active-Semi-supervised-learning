...
def get_dataset(args, algorithm, dataset, num_labels, num_classes, data_dir='./data', include_lb_to_ulb=True, lb_idx_mask=None, no_labels = False):
	"""
	create dataset
	...
	from semilearn.datasets import (get_eurosat, get_medmnist, get_semi_aves, get_cifar, get_svhn, get_stl10, get_imagenet, get_json_dset, get_pkl_dset, get_mnist, get_mnist_imb,
									get_plant300k, get_dollarstreet, get_grainset, get_wikichurch, get_sentinel2globallulc,
									get_babyear4k, get_species196, get_bioscan)
	...
	# SALOON dataset
		...
		lb_dset, ulb_dset, eval_dset = get_mnist(args, algorithm, num_labels, num_classes, data_dir=data_dir, include_lb_to_ulb=include_lb_to_ulb)
		test_dset = None
	elif dataset in ['bci_mnist', 'bcs_mnist', 'wci_mnist']:
		lb_dset, ulb_dset, eval_dset = get_mnist_imb(args, algorithm, dataset, num_labels, num_classes, data_dir=data_dir, include_lb_to_ulb=include_lb_to_ulb)
		test_dset = None
	elif dataset in ['plant300k']:
		lb_dset, ulb_dset, val_dset, eval_dset = get_plant300k(args, algorithm, dataset, num_labels, num_classes, data_dir=data_dir, include_lb_to_ulb=include_lb_to_ulb, lb_index=lb_index, ulb_index=ulb_index, no_labels =no_labels)
		test_dset = None
	elif dataset in ['dollarstreet_imagenet', 'dollarstreet_country', 'dollarstreet_income']:
		lb_dset, ulb_dset, eval_dset = get_dollarstreet(args, algorithm, dataset, num_labels, num_classes, data_dir=data_dir, include_lb_to_ulb=include_lb_to_ulb, lb_index=lb_index, ulb_index=ulb_index, no_labels =no_labels)
		test_dset = None
	elif dataset in ['grainset', 'grainset_maize', 'grainset_rice', 'grainset_wheat', 'grainset_sorghum']:
		lb_dset, ulb_dset, eval_dset = get_grainset(args, algorithm, dataset, num_labels, num_classes, data_dir=data_dir, include_lb_to_ulb=include_lb_to_ulb, lb_index=lb_index, ulb_index=ulb_index, no_labels =no_labels)
		test_dset = None
	elif dataset in ['wikichurch14']:
		lb_dset, ulb_dset, eval_dset = get_wikichurch(args, algorithm, dataset, num_labels, num_classes, data_dir=data_dir, include_lb_to_ulb=include_lb_to_ulb, lb_index=lb_index, ulb_index=ulb_index, no_labels =no_labels)
		test_dset = None
	elif dataset in ['sentinel2globallulc']:
		lb_dset, ulb_dset, eval_dset = get_sentinel2globallulc(args, algorithm, dataset, num_labels, num_classes, data_dir=data_dir, include_lb_to_ulb=include_lb_to_ulb, lb_index=lb_index, ulb_index=ulb_index, no_labels =no_labels)
		test_dset = None
	elif dataset in ['babyear4k']:
		lb_dset, ulb_dset, eval_dset = get_babyear4k(args, algorithm, dataset, num_labels, num_classes, data_dir=data_dir, include_lb_to_ulb=include_lb_to_ulb, lb_index=lb_index, ulb_index=ulb_index, no_labels =no_labels)
		test_dset = None
	elif dataset in ['species196']:
		lb_dset, ulb_dset, eval_dset = get_species196(args, algorithm, dataset, num_labels, num_classes, data_dir=data_dir, include_lb_to_ulb=include_lb_to_ulb, lb_index=lb_index, ulb_index=ulb_index, no_labels =no_labels)
		test_dset = None
	elif dataset in ['bioscan']:
		lb_dset, ulb_dset, test_dset, eval_dset = get_bioscan(args, algorithm, dataset, num_labels, num_classes, data_dir=data_dir, include_lb_to_ulb=include_lb_to_ulb, lb_index=lb_index, ulb_index=ulb_index, no_labels =no_labels)
	else:
		return None
