...
def get_config():
    ...
    ## al_algorithms data selection method
    parser.add_argument(
        "-al_alg", "--al_algorithm", type=str, default="random", help="al_algorithms learning algorithm"
    )
    parser.add_argument(
        "-alr", "--al_rounds", type=int, default=5, help="number of al rounds, equally distributed among num_labels"
    )
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    # todo: make sure that DS is initialized without labeled data, set it later
    ...
    # model.set_dataset()
    # query_budgets = args.al_rounds * [int(args.num_labels/args.al_rounds)]
    # ds_pool_size = ...  # todo
    # idxs_lb_mask = np.zeros(ds_pool_size, dtype=bool)
    # idxs_tmp = np.arange(ds_pool_size)
    # np.random.shuffle(idxs_tmp)
    # idxs_lb_mask[idxs_tmp[:query_budgets[0]]] = True
    # model.update_labeled_dataset(idxs_lb_mask)  # todo
    model.train_active()

    # print validation (and test results)
    for key, item in model.results_dict.items():
        logger.info(f"Model result AL round 0 (B: {np.sum(model.al.idxs_lb)}) - {key} : {item}")

    # # START AL Loop
    # for _round in np.arange(1, args.al_rounds):
    #     queried_idxs = active_learner.query()
    #     idxs_lb_mask[queried_idxs] = True
    #     model.update_labeled_dataset(idxs_lb_mask)  # todo
    #
    ...
    #     # print validation (and test results)
    #     for key, item in model.results_dict.items():
    #         logger.info(f"Model result AL round {_round} (B: {np.sum(idxs_lb_mask)}) - {key} : {item}")
    #
    #     active_learner.update_model(model)
